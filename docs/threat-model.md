...
| Path traversal | Symlink points outside repository | Never follow symlinks recorded in history, nor graft and alternate entries that are not regular files; use private temporary files only for bounded profile blobs | The operator-supplied repository path is deliberately resolved by the kernel; local filesystem race outside Tacita |
