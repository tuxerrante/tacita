...
kernel and filesystem, Git 2.43 or newer, the frozen optional Go 1.27.0 profile
...
| Repository target confusion | Supplied path is not a repository and Git discovers an ancestor one | Explicit `--git-dir`/`--work-tree` targeting instead of discovery; typed rejection of a path that is neither worktree nor bare | Operator may still name the wrong repository |
| Analysis-time network access and mutation | `diff-tree` over a partial clone lazily fetches missing objects and writes packs | Reject promisor and partial-clone repositories, from effective local and worktree configuration, before traversal | Git may add other lazy-fetch paths |
...
| Path traversal | Symlink points outside repository | Never follow symlinks recorded in history; reject graft and alternate entries that are not regular files; use private temporary files only for bounded profile blobs | The operator-supplied repository path is deliberately resolved by the kernel; local filesystem race outside Tacita |
